...
## SEO and GEO (search and AI answer engines)

The head carries the usual crawler signals plus the AI-answer-engine
equivalents, all static in `index.html` since a crawler that does not execute
JS only ever sees the raw HTML:

- **Favicon** — `assets/favicon.svg` plus rasterised PNGs
  (`favicon-32.png`, `apple-touch-icon.png`, `icon-512.png`), regenerated by
  `tools/generate-images.py`.
- **Canonical link, Open Graph, and Twitter Card** — `og:title` and
  `og:description` reuse `general.pageTitle` / `general.metaDescription` via
  `data-cms-content`, so they follow whatever the CMS holds. `og:image` points
  at `assets/images/social-card.png`, a static brand card (also
  `tools/generate-images.py`); it does not change per locale.
- **JSON-LD** (`<script type="application/ld+json">`) — an `Organization` node
  for Singhawat Concrete with address, phone, and product list, plus a
  `GeneralContractor` node for Singhawat Builders. This is the block most worth
  getting right for GEO: LLM answer engines lean on structured, unambiguous
  facts far more than they lean on prose.

### The fourth thing that must stay in agreement

JSON-LD is **hand-authored, not CMS-driven** — deliberately, so a crawler
without JS still sees it, which rules out having `cms.js` inject it after a
fetch. That means it is a fourth thing (alongside `data-cms`, `site.json`, and
`config.yml`) that can drift from the real content. `tools/verify-content.py`
catches what it mechanically can — the phone number and the LINE `sameAs` URL
against `content/site.json`'s `en` locale — but a change to the address or the
product list needs the JSON-LD block updated by hand.

`sameAs` currently lists LINE only. `contact.fbUrl` is still the placeholder
`https://facebook.com/` (see REQUIREMENTS.md), and a `sameAs` pointing at the
Facebook homepage would be actively wrong, not just incomplete — add it once
`fbUrl` is a real page.

### Known limitation: no hreflang

The three languages are switched client-side with no per-language URL, so
there is exactly one URL for `hreflang` alternates to point at — real
`hreflang` tags require distinct URLs per locale and would be lying if added
here. The practical consequence: **a crawler that does not run JS only ever
indexes the English fallback text**, and Google's JS-executing crawler
indexes whichever locale happens to be `currentLang` at crawl time (Thai, by
default). If ranking separately for Thai, English, and Chinese search terms
becomes important, the durable fix is per-locale URLs (`/th/`, `/en/`, `/zh/`
or `?lang=`) feeding the same `content/*.json`, which is a bigger change than
...
- **No `hreflang` alternates** — see "SEO and GEO" above; the single-URL,
  client-side language switch has no distinct URL per locale to point at.
- **JSON-LD `sameAs` has no Facebook entry** — `contact.fbUrl` is still a
  placeholder; add it once it is a real page.
